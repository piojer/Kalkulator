...
function wiersza($tekst, $a, $pole, $komentarz = '', $post = ' zł', $dod = '', $prefix = ''){
	echo "<tr><td>$tekst</td>";
	foreach ($a as $k){
		if ($komentarz != "")
...
function wiersza0($tekst, $a, $pole, $komentarz = '', $post = ' zł', $dod = '', $prefix = ''){
	echo "<tr><td>$tekst</td>";
...
function wiersz2wydatki($nazwa, $a, $key){
	...
	foreach ($a as $k){
		...
		$pod = myround($wyd->kwota - $wyd->kwotaNetto) . ' zł';
		$kom = "<h2>$nazwa</h2>";
		$kom .= "<table><tr><td>brutto:</td><td>$kw zł</td></td></tr>";
		foreach($wyd->wyniki->czastkowe as $n => $v){
			$kom .= "<tr><td>$n</td><td>".myround($v)." zł</td></td></tr>";
		}
		$kom .= "<tr><td>Prawdziwa wartość</td><td>".myround($wyd->kwotaNetto)." zł</td></td></tr>";
		$kom .= "</table>";
		$pod = komentarz($pod, $kom);
		echo "<td>$kw zł</td><td class='dodatek'><div class='dodatek'>$pod</div></td>";
	}
	echo "</tr>";
}
...
function wyswietlKilka(array $a) {
	...
	wierszWyniki("Składki na ZUS i PIT ", 'pracow', $a, 1);
	...
	wierszp("inne umowy", "będą wkrótce :)");
	//wierszp("widok", "<select name='widok'><option value='domyslny'>domyślny</option><option value='pelny'>pełny</option><option value='zwarty'>zwarty</option></select>");
	...
	wiersza("Suma podatków, opłat, itp.", $a, 'sumaPodatkow');
	...
	echo "</tr>";
	wiersza0("Miesięczny zysk", $a, 'zysk');
	wiersza0("Za swoje wynagrodzenie kupisz", $a, 'ilePWiecej', '', '% więcej', '', 'o ');
	echo "</table></form>";
