...
$komentarz = "Źródło: <a href='http://wynagrodzenia.pl/kalkulator_oblicz.php'>wynagrodzenia.pl</a>";
...
$podatkiPrac[] = new PodatekLiniowyZKwota("ZUS", "FGŚP", $komentarz, 0.00144, -2.33);
...
$kosztyPozwolen = new PodatekProcentowy("pozwolenia", "Koszty pozwoleń i uzgodnień", "W koszcie wynajmu/kredytu są sztuczne obciążenia przy budowie: Pozwolenie na budowe, na wycinki drzew. Projektant, architekt, kierownik budowy to zawody licencjonowane, więc narzucają wyższe ceny. Dodatkowo są uzgodnienia z monopolistami: dostawcami wody, prądu, gazu. (Trzeba do nich się podłączyć by uzyskać pozwolenie na budowę) Przy budowie domku jednorodzinnego te koszty to ponad 30 tys. zł, co najmniej połowa bezsensowna, więc zakładam narzut 5%", 5);
$kosztyKredytow = new PodatekProcentowy("inflacja", "Koszty kredytu/inflacji", "Zakładam wkład własny ok. 40%, koszty kredytu +100%, pomniejszam o inflację. W przypadku oszczędzania na budowę nieco mniej straci się przez inflację", 30);
$kosztySztucznegoPosrednika = new PodatekProcentowy("monopol", "Koszty pośrednika", "Rolnik nie może sobie legalnie sprzedać bezpośrednio do sklepu, tylko musi do pośrednika (regulacje UE). No i np. cena skupu mleka to 1,1zł/l <a href=\"http://www.mleczarstwopolskie.pl/cgblog/1673/54/Ceny-skupu-mleka-w-czerwcu-2015-r\">z czerwca 2015</a>, a cena mleka świeżego w sklepach to min 1,95 zł/l. Rolnik sprzedając bezpośrednio do sklepu miałby koszt maszyny pasteryzującej/pakującej 100 tys zł/3 lat/1000litrów/dzień = 10 gr/l + butelka 10 gr, Oczywiście sklepy maja narzut ok. 20%, czyli cena w sklepie byłaby 1,55 zł/l. Pośrednik pobiera więc min 20% ceny", 20);

$lokalnyMonopol = new PodatekProcentowy("monopol", "Lokany monopol", "", 30);
$oligopol = new PodatekProcentowy("monopol", "Oligopol", "", 20);
$dopuszczenie = new PodatekProcentowy("pozwolenia", "Dopuszczenie do sprzedaży", "", 30);
$akcyza = new PodatekProcentowy("Akcyzy", "Akcyza", "", 39.4);
$oplataPaliwowa = new PodatekProcentowy("Akcyzy", "Opłata paliwowa", "", 5.4778);


$podatkiWZakupach[] = new PodatekProcentowy("dochodowe", "Podatek dochodowy sprzedawcy", "Pracodawcy płacą około 19% ze swoich zysków podatku dochodowego. Gdyby tego podatku nie było, konkurencja na rynku wymusiłaby obniżke cen o ten podatek. Zakładając średni zysk firm około 5%, ceny spadłyby o około 1%", 1);
$podatkiWZakupach[] = new PodatekProcentowy("biurokracja", "Koszty biurokracji", "Według oszacowań około 2.15% wszyskich kosztów firm to sprawy związane z obsługą urzedników i urzędów ", 2.15);
...
$wydatki[] = $a = new GrupaWydatkow("Żywność", $podatkiWZakupach, 400/2050);
...
$wydatki[] = $a = new GrupaWydatkow("Wyposażenie mieszkania", $podatkiWZakupach, 50/2050);
...
$podatkiOdWyn[] = new PodatekKwotowy("biurokracja", "Mandaty i opłaty skarbowe", "Według Rocznika Statystycznego GUS w 2013 r  pobrano 19431 mln z tytułu Opłaty, grzywny, odsetki i inne
dochody niepodatkowe. Tą kwotę dzielimy na 31 mln dorosłych ludzi (https://pl.wikipedia.org/wiki/Ludność_Polski)", 19431/31/12);
...
wiersz("Płaci pracodawca<br/><b>(prawdziwe brutto)</b>", $brutto);
...
wiersz("Składki na ZUS " . rozwiniecie('pracow'), $podDoWyn); //"Składki po stronie pracownika "
...
wiersz2p("Suma podatków, opłat, itp.", myround(100*(1- $sumaNetto/$brutto)).'%', "");
echo "</table>";
...
//if (isset($_GET['netto'])) {
	...
	*/
	...
	obnizenie('jakVat', 50, 25, $kwota, 'Licencje i monopole', "Bardzo dużo produktów, które kupujemy jest ograniczonych licencjami, pozwoleniami na produkcję, oraz państwowymi monopolami. <br\>
	...
	- Produkty spożywcze (czyli 23% produktów jak z VATu), <br/> 
	- Budowlane (trzeba mieć pozwolenia na budowę), więc koszty mieszkań i czynsze też<br/>
