...
function wiersza($tekst, $a, $pole, $komentarz = '', $post = ' zł', $dod = ''){
	echo "<tr><td>$tekst</td>";
	foreach ($a as $k){
		if ($komentarz != "")
...
function wiersz2wydatki($nazwa, $a, $key){
	...
	foreach ($a as $k){
		...
		foreach($wyd->wyniki->czastkowe as $nazwa => $v){
			$kom .= "<tr><td>$nazwa</td><td>".myround($v)." zł</td></td></tr>";
		}
...
function wyswietlKilka(array $a) {
	...
	wiersza("Za swoje wynagrodzenie kupisz", $a, 'ilePWiecej', '', '% <br/> więcej');
	echo "</table></form>";
